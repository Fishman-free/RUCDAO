...
RUCDAO 把全校志愿项目整合到一个平台：学生自由浏览报名，参与后获得**米粒（RUCOIN）**——一种**不可转让的志愿服务贡献凭证**（对标乡建DAO 的"稻米"）。米粒兑换权益、抵扣活动费用、点亮链上 SBT 徽章；志愿时长经平台**直通车如实录入"志愿北京"**。

**三条铁律**：米粒不可转让/不可兑换现金 · 时长只按真实服务录入 · 发放规则公开共治。
...
└── web/                          # 平台 v0 可交互原型（广场 / 项目库 / 权益商店 / 我的）
    ├── index.html
    └── assets/{css,js}/
```
