...
RUCDAO 把全校志愿项目整合到一个平台：学生自由浏览报名，参与即产出 **PoV 贡献证明**，获得**米粒（RUCOIN）**——一种**与人民币不挂钩、不可充值、不可转让的志愿服务贡献凭证**（对标乡建DAO 的"稻米"）。米粒可领**面包券等伙伴心意券**、兑发展类权益、点亮链上 SBT 徽章——答谢是心意不是报酬（实物答谢设学期上限）；志愿时长经平台**直通车如实录入"志愿北京"**，与答谢彻底解耦。

**RUCOIN 就是志愿时数的认证方式**：1 小时 = 10 粒，余额即认证时数账本，与志愿北京时数对标。

**双端架构**：**学生端**（广场 / 项目库 / 伙伴答谢 / 米粒钱包 / 时数核对 / 我的）寻找并完成志愿、领心意、对账时数；**发布端**（发布台 / 项目管理 / 发放台）发布志愿、确认服务、一键发放时数（入账 + 链上存证 + 志愿北京批量录入）。
